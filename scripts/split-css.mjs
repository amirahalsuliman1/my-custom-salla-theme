...
/**
 * ⚠ REFUSE TO RUN TWICE. Found on 2026-08-12 by doing exactly this by hand.
 *
 * The script is not idempotent and cannot be: its input is the unsplit sheet,
 * and its first run consumes it. Run it a second time and the head has no
 * platform rules left, so it writes a **valid, almost-empty**
 * `salla-components.css` over the real one — 363 KB of the platform's component
 * CSS replaced by a banner, with a cheerful «0 platform rules moved» and exit 0.
 *
 * Nothing downstream notices. `check-budgets.mjs` sees a sheet far under its
 * ceiling and passes; the tests read templates, not `public/`. The failure
 * surfaces on a live store, as the platform's cart and filters rendering
 * unstyled — which is the exact outcome the header of this file says must never
 * happen quietly.
 *
 * The webpack plugin always runs this immediately after emit, so the normal
 * build is unaffected. This guard is for the hand-run.
 */
if (movedCount === 0 && fs.existsSync(TARGET)) {
  console.error('✗ split-css: no platform rules found above the marker.')
  console.error('  `app.css` has already been split — this looks like a second run.')
  console.error('  Writing now would replace salla-components.css with an empty sheet.')
  console.error('  Nothing was written. Rebuild first: pnpm run production')
  process.exit(1)
}
...
/**
 * ⚠ THE BANNER WAS REWRITTEN ON 2026-08-12 BECAUSE IT WAS MISREAD, AND THE
 * MISREADING WAS THE BANNER'S FAULT RATHER THAN THE READER'S.
 *
 * It used to print a line labelled «before» carrying a bare `app.css` and a
 * number above the budget ceiling. That number is the **unsplit intermediate**
 * — a file that exists for the few milliseconds between webpack emitting it and
 * this script cutting it in two, and which **is never served to anybody**. Read
 * quickly, and next to a filename that IS shipped, it looks exactly like a
 * budget breach; the project owner reported one on that basis.
 *
 * So the intermediate now says what it is on its own line, the shipped files are
 * marked, and the banner points at the script that actually decides. A build
 * log is read far more often than the script that prints it, and a line that
 * invites a wrong conclusion costs more than the information it carries.
 */
const beforeGz = gzip(css)
console.log('CSS split — T-8.01\n')
console.log(
  `  intermediate    app.css (unsplit)    ${kb(css.length).padStart(9)} raw  ${kb(beforeGz).padStart(9)} gzip   ← never shipped`,
)
console.log(
  ...
  `  SHIPPED         app.css              ${kb(appOut.length).padStart(9)} raw  ${kb(gzip(appOut)).padStart(9)} gzip   deferred`,
)
console.log(
  `  SHIPPED         salla-components.css ${kb(sallaOut.length).padStart(9)} raw  ${kb(gzip(sallaOut)).padStart(9)} gzip   deferred`,
)
console.log(
  `\n  ${movedCount} platform rules moved. Every one is still shipped — see the header of this file.`,
)
console.log(
  '  Budgets are decided by `node scripts/check-budgets.mjs`, against the SHIPPED\n' +
    '  rows only. The intermediate above is not budgeted and cannot breach anything.',
)
