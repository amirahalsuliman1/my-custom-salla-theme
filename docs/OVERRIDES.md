...
| `webpack.config.js` | A | `1.365.0` | T-2.10 — **one file added to the `app` entry array**, `js/partials/bottom-sheet.js`, and nothing else. The overlay primitive has to be in the bundle every page loads, and importing it from `app.js` would have adopted that file — and its sixteen pre-existing lint problems — under the T-1.07 ratchet, for one line. Reconcile by taking upstream's config and re-adding the array element | — |
