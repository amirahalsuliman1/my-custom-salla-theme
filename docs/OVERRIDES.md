...
| `src/views/pages/customer/orders/single.twig` | A | `1.365.0` | T-6.02 — **re-skinned, not re-scoped.** The page has no artboard (the two the backlog named are Salla's checkout — AC-10), so the owner ruled it derived from T-6.01's components: every block is `.account-panel`, the rows are `components.orders.item`, the totals are `salla-order-totals-card`. **Every `salla-*` element upstream mounted is still mounted** — `salla-order-details`, `salla-edit-order-button`, `salla-order-branch`, `salla-review-factors-tags`, `salla-rating-stars`, and both `salla-modal`s, which `order.js` still binds until T-6.03 and T-6.04 replace them. Three unreachable hover tooltips became announced text and four `target="_blank"` links gained `rel="noopener"`. Reconcile by re-reading upstream for **new `salla-*` elements, new hooks and new order fields**, not for layout | — |
| `src/assets/js/order.js` — **DELETED** | A | `1.365.0` | T-6.03 / T-6.04 — the file is gone from the tree. It contained exactly two `app.onClick` bindings, `#btn-reorder` and `#confirm-cancel`, both targeting `salla-button`s inside `salla-modal`s this theme no longer renders, and **both calling their API with no order id** — survivable on a page about one order, wrong the moment the orders list offers the same actions on twenty. `partials/order-cancel.js` and `partials/order-reorder.js` replace them, pass the id read off the trigger, and serve both pages. **On upgrade, re-read upstream's `order.js` for NEW bindings** — anything it gains must be reproduced in those two partials or deliberately dropped. The webpack entry keeps the name `order`, so both templates' `<script src="order.js">` is unchanged | `webpack.config.js` |
| `src/views/pages/thank-you.twig` | A | `1.365.0` | T-6.07 — derived, because its named artboard turned out to be T-6.08's rating success screen. Rebuilt on `.account-panel` and the account shell's **classes** — deliberately **not** `layouts.customer`, whose `user.type == 'user'` guard would tell a guest who has just paid to sign in. **Every `salla-*` element upstream mounted is still mounted**: `salla-order-shipments`, `salla-next-order-coupon`, `salla-order-branch`, and the invoice form still submits through `salla.form.onSubmit`. Two defects fixed: `opacity-0` made the whole confirmation invisible without JS, and the resend-invoice input had a placeholder and no label. Upstream's brand-coloured wave banner is the one thing removed. `user-pages.scss` is untouched — its `.thankyou-block` rules are overridden by import order. Reconcile by re-reading upstream for **new `salla-*` elements and new hooks** | `src/assets/styles/04-components/order-card.scss` |
| `src/views/pages/page-single.twig` | A | `1.365.0` | T-7.01 — the shared CMS shell. The prose branch is rebuilt on `.account-panel` with prose styles declared once in `04-components/prose.scss`; **the `customised` branch keeps upstream's `information_page` hook exactly as it was**, because that hook is Salla's own mechanism for component-composed pages and the stories feed rides on it. Adds a breadcrumb to both branches and, on the page named by `stories_page_id`, a mount point for the category chips and brand dropdown. `salla-comments` is still mounted on ordinary pages. Reconcile by re-reading upstream for **changes to the hook name and to `page.type` values** | `src/assets/js/stories.js`, `webpack.config.js` |
