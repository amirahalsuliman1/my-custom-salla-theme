...
- **What was done:**
  ...
  - **Two things the design draws are not built, and the reasons differ.** The **quick-view control** belongs to T-4.13 and is deliberately absent rather than rendered inert — a button that does nothing is worse than no button for someone who cannot see that it is dead. The **wishlist heart** is absent because **no artboard draws one**; the design puts quick view in that corner instead. Both are recorded, and the heart's removal is flagged as the weaker inference of the two: absence is thinner evidence than presence.
  ...
  - **Swatches are presentational, which contradicts this task's own acceptance criterion** and is recorded as a contradiction rather than quietly resolved. No artboard draws a selected state on a card swatch, and T-2.15's shell is built on **exactly one focusable descendant per card** — making five dots tabbable would undo the thing that shell exists to do. Each dot carries its colour name in visually-hidden text, so colour is never the only channel.
...
#### T-4.08 — Home page assembly — 🟡 **PARTIAL 2026-08-08** — the two carried items are closed; the assembly itself waits on its missing sections
...
- **What was done — and what deliberately was not:**
  ...
  - **What remains before this task can close.** The sections themselves: **T-4.03** (Winter Is Coming carousel), **T-4.06** (two shoppable blocks), **T-7.06** (تجارب عملائنا stories), **T-4.21** (تنسيقات جاهزة من أملاس), and the partner CTA banner recorded against this task in `/docs/DERIVED-DECISIONS.md`. Plus **T-3.03**, whose marquee sits above the header and must set `--header-offset`. Also unaddressed: **"below-fold sections lazy-load"**, which cannot be verified with one section on the page, and **"matches both Home artboards"**, which cannot be checked against sections that do not exist. **None of these were skipped quietly — they are listed here so the 🟡 is legible.**
