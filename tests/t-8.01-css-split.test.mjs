...
describe('T-8.01 — the shipped build', () => {
  test('master.twig links the deferred sheet BEFORE app.css', () => {
    ...
    assert.ok(salla < app, 'cascade order broken — the deferred sheet must come first in the document');
  });
  ...
  test('it is loaded non-blocking, with a noscript fallback that is not', () => {
    ...
    assert.match(master, /<noscript><link rel="stylesheet" href="\{\{ 'salla-components\.css' \| asset \}\}"><\/noscript>/);
  });
});
